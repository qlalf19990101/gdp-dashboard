...
if option == "소형 재물 획득의 비약":
    ...
    if st.button("계산"):
        ...
        st.success(f"피로도 5당 이익: {money_per_fatigue:.2f}만 메소")
        st.success(f"피로도 500 소진 시 총 이익: {total_money:.2f}만 메소")
    ...
    if st.button("계산"):
        ...
        st.success(f"피로도 10당 이익: {profit_per_10_fatigue:.2f}만 메소")
        st.success(f"피로도 500 소진 시 총 이익: {total_boss_profit:.2f}만 메소")
    ...
    if st.button("계산"):
        ...
        st.success(f"피로도 5당 이익: {profit_per_5_fatigue:.2f}만 메소")
        st.success(f"피로도 500 소진 시 총 이익: {total_exp_profit:.2f}만 메소")
